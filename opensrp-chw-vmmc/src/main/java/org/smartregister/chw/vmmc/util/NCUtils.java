...
public class NCUtils {
    ...
    public static org.smartregister.Context context() {
        return VmmcLibrary.getInstance().context();
    }
    ...
    public static SimpleDateFormat getSourceDateFormat() {
        return new SimpleDateFormat(VmmcLibrary.getInstance().getSourceDateFormat(), Locale.getDefault());
    }

    public static SimpleDateFormat getSaveDateFormat() {
        return new SimpleDateFormat(VmmcLibrary.getInstance().getSaveDateFormat(), Locale.getDefault());
    }
    ...
    public static void processHomeVisit(EventClient baseEvent, SQLiteDatabase database, String parentEventType) {
        try {
            Visit visit = VmmcLibrary.getInstance().visitRepository().getVisitByFormSubmissionID(baseEvent.getEvent().getFormSubmissionId());
            if (visit == null) {
                ...
                if (StringUtils.isNotBlank(parentEventType) && !parentEventType.equalsIgnoreCase(visit.getVisitType())) {
                    String parentVisitID = VmmcLibrary.getInstance().visitRepository().getParentVisitEventID(visit.getBaseEntityId(), parentEventType, visit.getDate());
                    visit.setParentVisitID(parentVisitID);
                }

                if (database != null) {
                    VmmcLibrary.getInstance().visitRepository().addVisit(visit, database);
                } else {
                    VmmcLibrary.getInstance().visitRepository().addVisit(visit);
                }
                    ...
                    for (Map.Entry<String, List<VisitDetail>> entry : visit.getVisitDetails().entrySet()) {
                        if (entry.getValue() != null) {
                            for (VisitDetail detail : entry.getValue()) {
                                if (database != null) {
                                    VmmcLibrary.getInstance().visitDetailsRepository().addVisitDetails(detail, database);
                                } else {
                                    VmmcLibrary.getInstance().visitDetailsRepository().addVisitDetails(detail);
                                }
                            }
                        }
                    }
